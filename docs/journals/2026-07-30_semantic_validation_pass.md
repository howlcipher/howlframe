...
Next, extend propagation through `try_let`, `for`, `spawn`, `match`, and backend-specific primitives, then add a backend that uses `IRNode.Type` to select native layouts. Do not mark #64 done until those decisions are verified end to end.
