...
```
$ go run zero.go -run tests/test_advanced_control.zero
{"reason":"\"match\" is not supported under -run in Phase 1 ..."}   exit 1   # interpreter fails closed
$ go run zero.go -compile-bc tests/test_advanced_control.zero -o /tmp/m.bin && go run zero.go -run-bc /tmp/m.bin
...
open head set; `runZirGate` only blocks on `ZIR_INVALID_REF`/`ZIR_TARGET_INFEASIBLE` and `isFeasible` had rules
...
| `internal/zir/constructs.go` | **new** — `VerifyConstructs` emits `ZIR_TARGET_INFEASIBLE` |
| `internal/zir/constructs_test.go` | **new** — target scoping, diagnostic shape, sub-form acceptance |
...
| `zero.go` | `runZirGate` merges construct diagnostics into the existing blocking-code filter |
| `zero_test.go` | 6 new CLI-level tests, including the corpus partition guard |
...
Design: `internal/construct` (AST → violations) ← `internal/zir/constructs.go` (violations → diagnostics) ←
`zero.go` `runZirGate`. `internal/construct` imports only `internal/ast`, mirroring `internal/capability`'s
backend-independence (the precedent from improvement #94). `internal/zir` does not import `internal/bytecode`.

## The finding that changed the design

`bugs.md` #45's fix sketch prescribed a deny-list over **ZIR node kinds** inside `isFeasible`. That matching
...
1. **`catch` is a sub-form of the supported `try_let`.** `bytecode.go:136-149` destructures `node.Children[2]`
   itself, so `catch` never reaches `compileNode` in head position — but `zir.LowerAST` (`lowering.go:64-66`)
...
3. **`tests/optimization_signature.zero` uses `(test "go test ./...")` as `optimize_signature` metadata**, not
...
`TestZirGateAcceptsAllExistingFixtures` (`zero_test.go:919`) was the designated empirical check. It runs
`-validate`, which passes `zirTargetNone` (`""`), and `Verify` only applies target rules for a non-empty
...
```
go build -o <scratch>/zerocheck zero.go     OK
go vet ./...                                OK
...
| Command | Result |
| --- | --- |
| `-compile-bc tests/test_advanced_control.zero` | fails, `ZIR_TARGET_INFEASIBLE` naming `match`, **no artifact** |
| `-compile-bc tests/test_void_defun.zero` | fails, naming `test` and citing `improvements.md #96`, no artifact |
| `-compile-bc` on an invented head | fails, naming the head, no artifact |
| `-compile-bc examples/cli_hello.zero` → `-run-bc` | succeeds, prints `Hello, World!` / `Welcome to Zero` |
| `-compile-bc tests/optimization_signature.zero` | still succeeds (its `(test ...)` is metadata) |
...
`tests/test_return_compound.zero` across three consecutive runs. That artifact nondeterminism is pre-existing
...
`test_trace`, `test_void_defun`, `test.zero`, and `examples/wasm_math`. Every one is already exempt in
`tools/difftest/manifest.json` or skipped by the ZIR fixture test, which is why difftest is unchanged. Each
names a construct with genuine runtime meaning the VM cannot express — that visibility is the deliverable.

`tests/routes.zero` and `tests/test_include.zero` still fail earlier, in `checker.Check`, on bug #43's
...
comes from the AST, not ZIR (#90). No opcode was added, removed, or renamed, so
`docs/reference/bytecode_reference.md` needed no regeneration. `zirBlockingCodes`' policy is unchanged and
`ZIR_UNBOUND_REF` remains excluded (#42). The gate was not extended to `-validate`, which is target-independent
