...
## Context
During ZIR wiring (improvement #87 Phase 2), we noticed that `internal/zir/verifier.go` had a hardcoded `getCapability` switch mapped ZIR node kinds to capability types. The bytecode compiler `internal/bytecode/opcode.go` had its own capability configuration for opcodes. The two representations were completely independent and ran the risk of silent divergence if a new capability was added to one but not the other. 
...
## Design Decisions
1. **Target-independent location**: I created the new `zero/internal/capability` package as the source of truth for the capability enum (`capability.Capability`) and its canonical mapping.
2. **Direction**: The dependency direction flows from both `internal/zir` and `internal/bytecode` into `internal/capability`. This preserves the backend independence of the ZIR compiler and bytecode systems.
3. **Construct Mapping**: `capability.ForConstruct(name string)` was implemented as the single mapping from canonical Zero operation strings to capabilities. `internal/zir` accesses this directly using `node.Kind`.
