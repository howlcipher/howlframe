...
(`go build -o <scratch>/zerobin zero.go`, go1.26.5) and every claim below was reproduced against tracked
fixtures. Code was read directly in `zero.go`, `internal/ast`, `internal/parser`, `internal/checker`,
`internal/zir`, `internal/bytecode`, `internal/vm`, `internal/capability`, `tools/difftest`,
...
Reproduced against `tests/test_advanced_control.zero`, which should print `zero`, `one`, `other`:

```
$ ./zerobin -run tests/test_advanced_control.zero
{"reason":"\"match\" is not supported under -run in Phase 1 (...)","line":5,"column":6}
exit 1                                       # correct — the interpreter fails closed

$ ./zerobin -compile-bc tests/test_advanced_control.zero -o m.bin && ./zerobin -run-bc m.bin
exit 0                                       # WRONG — no output at all, artifact written
```

`tests/test_void_defun.zero` behaves identically (its `test` block vanishes). An invented head passes
every source-based gate:

```
$ ./zerobin -validate   t_bogus.zero        # exit 0
$ ./zerobin -compile-bc t_bogus.zero        # exit 0
$ ./zerobin -run-bc     b.bin               # prints before/after, skips (totally_made_up_head ...), exit 0
```
...
- `zero.go`'s `runZirGate` blocks only on `ZIR_INVALID_REF` / `ZIR_TARGET_INFEASIBLE`, and
  `internal/zir/verifier.go`'s `isFeasible` has real rejection rules **only** for `target == "wasm"`.
...
### F2 — ZIR is a shadow pass, not the lowering source

The blueprint's target path is `ZIR -> verifier -> lowered ZIR -> bytecode -> VM`. The code does
`AST -> bytecode`: `zero.go:107` calls `bytecode.CompileToBytecode(root)` on the AST, with `runZirGate`
verifying a separately-lowered graph alongside. No backend consumes ZIR. `zir.LowerAST` is a near-1:1 AST
mirror whose `ControlEdges` field is declared but never populated.

Improvements #86 and #87 are legitimately Done for what they scoped. But "ZIR owns semantic meaning" is
...
A consequence that matters for #45's fix: `zir.LowerAST` derives `Kind` from the head symbol of *every*
list, so `(let (x 0) body)` produces a ZIR node with `Kind == "x"`, and parameter lists, dict pairs, and
match arms produce kinds named after user identifiers and literals. Any ZIR-side rejection rule must be an
...
| Dimension | Maturity |
| --- | --- |
| Standalone runtime | Early, and untrustworthy at the boundary. Deterministic core, `cli_args`, `read_line`/`stderr`/`exit` (#97), file ops, JSON parse, and deny-by-default capability enforcement all reach the VM — but unsupported constructs fail **open** (F1), modules and `test` blocks are absent from ZIR/bytecode/VM, artifacts are bare `gob`, and there is no project lifecycle, packaging, or release story. |
| AI-native runtime | Primitive-level only. AI opcodes exist but hardcode Ollama. No provider-neutral adapter (#88), no schema-constrained output boundary, no budgets, no provenance, no deterministic test doubles, no bounded repair (#91). AI operations are opcodes, not ZIR effects. |
...
- Updated **#90's Description** with the F2 finding (ZIR is off the execution path).
...
hashing/versioning → #98; model-adapter neutrality → #88; bounded semantic repair → #91; ZIR/backend
...
3. #95 — modules in ZIR + bytecode + VM
4. #96 — native VM `test` block execution
5. #98 — artifact validation, hashing, versioning
6. Bug #42 — `ZIR_UNBOUND_REF` false positives
7. #99 — generated coverage matrix
8. #88 — provider-neutral ZIR model adapter
9. #100 — structured experience memory
10. #90 / #89 / #91 — lowered-ZIR ABI, content-addressed ZIR, semantic deltas
...
has a ready-made regression fixture in `tests/test_advanced_control.zero`.

## Confirmation

No implementation code was changed in this session. Nothing under `internal/`, `tools/`, `cmd/`, `tests/`,
`examples/`, `benchmarks/`, `docs/reference/`, `.github/`, `zero.go`, or `zero_test.go` was modified. The
