...
## Description
Bug #43 reported that `(use "routes.zero" as routes)` failed to resolve an imported function: `tests/test_include.zero` failed `checker.Check` with `undefined reference to function "getGreeting"`, where `getGreeting` is exported by `tests/routes.zero`.
...
- A **source** module, as written in a `.zero` file, is *unnamed*: `(module stmt...)`. Its statements begin at `Children[1]`.
- The **synthetic** module `ExpandIncludes` builds when rewriting `(use "mod.zero" as m)` is *named*: `(module "m" stmt...)`. Its statements begin at `Children[2]`.
...
That silently discarded the first statement of every included module. In `tests/routes.zero` the first statement is the whole `(export (defun getGreeting ...))` block, so `getGreeting` never entered the importing file's scope, while the second statement `(route "/greet" ...)` survived intact — producing a failure that looked export-specific but was purely positional.
...
## Verification
- `./zero -validate tests/test_include.zero` and `./zero -compile-bc tests/test_include.zero` both exit 0.
...
## Exemptions Removed
`tests/test_include.zero` was excluded from `TestZirGateAcceptsAllExistingFixtures` and `TestCompileBcCorpusPartitionMatchesRegistry` as a documented pre-existing failure. Both exclusions are gone; the fixture is now covered unexempted.

`tests/routes.zero` remains skipped in both, for its own unrelated reason — it is an include-only fragment with no standalone root, valid only when pulled in by an importer. The comments in `zero_test.go` that attributed its skip to bug #43 were stale and have been corrected.

## Fixed in Passing
`TestCompileBcCorpusPartitionMatchesRegistry` ran `construct.Scan` against a **raw** parse, while the real `-compile-bc` path scans a fully expanded AST (`zero.go:74-77` runs `ExpandIncludes` and `ResolveModules` before `runZirGate` at line 106). While `test_include.zero` was exempt this mismatch was unreachable; removing the exemption exposed it, since the test would have seen a `use` head that the compiler never sees. The test now expands includes and resolves modules before scanning, restoring parity with the production pipeline.
