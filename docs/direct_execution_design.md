...
**Implementation note:** this lives in `howlframe.go` itself, not a separate file. The documented `go run howlframe.go yourfile.howl` invocation compiles only the named Go source file, so moving `Interpret` into another root file would either leave that symbol undefined or require every caller to name both files. Keeping the interpreter in the compiler entry point preserves the single-file invocation while allowing ordinary package-wide build, vet, and test commands to include the compiler normally.

Wired into `main()` via a new `-run` boolean flag:
- `./howlframe -run script.howl [args...]` — interpret and execute immediately, no files written.
- `./howlframe script.howl` (no `-run`) — unchanged: compile through the Go backend to `server.go`/`server_test.go` as before.
...
This is a deliberate simplification, not an oversight: one of #49's real motivations (see improvement #46's `type_hint` token-overhead finding) is that Go's static typing forces AI-authored HowlFrame code to spend tokens on `type_hint` boilerplate purely to satisfy the *codegen target*, not the logic itself. An interpreter with no compilation step has no such requirement — `defun` parameters and `list`/`dict` elements simply hold whatever value flows into them at call/construction time.

### Two deliberate deviations from the Go backend (not bugs)

1. **`list`/`dict` element types.** The Go backend's `let`-embedded `list`/`dict` codegen (howlframe.go ~1235–1260) hardcodes `[]string{...}` — every element becomes a Go string literal regardless of source type. The interpreter has no such restriction; elements keep their native type (`int64`, `string`, nested collections).
...
(An earlier draft of this document claimed a third deviation — `if`/`while` condition shape, citing bug #18 as still-open for compound/`and`/`or` conditions. Verified empirically during implementation that bug #18 is actually fixed for both `if` and `while` in the current `howlframe.go`; the claim was based on a stale detail note in `bugs.md` that contradicted its own ranked-table "Done" status. Removed the false claim here; see the 2026-07-24 journal and the bugs.md groom pass for the correction.)
...
For every node kind Phase 1 covers, behavior must be provably identical to the existing Go backend's observable output (stdout), not just "plausible." The verification fixture (`tests/test_interpret_basic.howl`) is run two ways and diffed:

```
./howlframe -run tests/test_interpret_basic.howl            # Phase 1 interpreter, no files written
./howlframe tests/test_interpret_basic.howl && go run server.go   # existing Go backend, unchanged
```
