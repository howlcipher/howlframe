...
| 49 | [Direct Neural Bytecode Synthesis](#49-direct-neural-bytecode-synthesis) | Done (2026-07-29) | 1.00 (8×1.0÷8) | Sonnet 5 | — | Monumental shift; massive effort but maximum value. |
| 61 | [Direct Binary Bytecode Serialization](#61-direct-binary-bytecode-serialization) | Done (2026-07-29) | 1.0 (8×1.0÷8) | Sonnet 5 | — | Replaces the legacy human-readable JSON bytecode with a fully binary Go gob encoding. |
...
* **Done (2026-07-27):** Added the `wasm_app` root and `generateWasmCode`/`emitWasmIR` in `zero.go`. It emits a standards-compliant `app.wat` module exporting `main(): i32` for a deliberately portable integer subset: literals, arithmetic/comparisons, `if` with both branches, `do`, and `return`. Unsupported nodes fail with a clear compiler diagnostic instead of being silently miscompiled. `examples/wasm_math.zero` and subprocess tests cover output shape and rejection behavior. `go test ./...`, `go vet zero.go`, and the standalone Go fixture compile loop pass. No WAT validator was installed, so `.wasm` compilation/instantiation remains optional external-tool verification rather than a claimed result.
