...
| 93 | [Module system Phase 2: implementation of use/export](#93-module-system-phase-2-implementation-of-useexport) | Done (2026-08-05) | 1.0 (6×1÷6) | — | Gemini 3.1 Pro | — | Implement the module system designed in Phase 1 (`docs/module_system_design.md`). Requires modifying parser, checker, and all backend emitters to support explicit exports and namespace-prefixed usage. |
...
* **Done (2026-08-04):** Implemented GitHub Actions CI workflow in `.github/workflows/ci.yml`, verifying go format, build, vet, and test across the repository.
...
* **Status Note:** Done (2026-08-05). `internal/vm.BCVM.run` now checks each instruction's `bytecode.Registry[op].Capability` against a new `AllowedCaps` field before dispatch, panicking with a structured `CAPABILITY_DENIED` `VMError` when it's absent. Default is fail-closed (nil/empty allow-list denies every capability-gated instruction; `CapNone` instructions are always permitted). `zero.go` gained a `-allow-caps` flag (comma-separated, validated against the known capability set, must precede the positional input file like every other `zero.go` flag) that builds the allow-list passed into `vm.RunBytecode`. Tests: `internal/vm/capability_test.go` (allowed/denied/denied-by-other-caps per capability kind, plus a CapNone-is-never-denied case) and `zero_test.go`'s `TestRunBytecodeAllowCapsFlagGatesCapabilities` (CLI-level: default denial, granted access, and rejection of an unknown capability name). `internal/vm/store_test.go`'s `newStoreTestVM` was updated to grant `CapDatabase` since store opcodes are capability-gated. `tools/difftest/main.go`'s `-run-bc` step now passes all five capabilities so differential backend-parity testing isn't affected by the new default-deny behavior. Documented in README.md's new "Capability Security" section.
...
* **Status Note:** Done (2026-08-05). Added internal/zir package with versioned graph, stable node IDs, typed ports, provenance, and deterministic serialization. Implemented AST-to-ZIR compatibility lowering.
...
* **Status Note:** Done (2026-08-05). Added pre-emission verification for ZIR graph, handling types, invariants, effect inference, and target feasibility in `internal/zir/verifier.go`. Included unbound reference diagnostics (#77) and effect capability inference (#79).
...
* **Done (2026-08-04):** Implemented the cross-backend testing harness in `tools/difftest/main.go`. It successfully exercises the interpreter, Go backend, and bytecode VM paths on `tests/*.zero`. Added an explicit `manifest.json` exclusion list to reliably skip `(web_app)`, `(http_server)`, `(module)` roots, and fixtures containing unsupported Phase-1 `-run` syntax. The runner explicitly caught two silent stdout divergences in `test_cli.zero` and `test_collection_read.zero` which were documented as bug #29.

### 93. Module system Phase 2: implementation of use/export
* **Description:** Implement the module system designed in Phase 1 (docs/module_system_design.md). Requires modifying parser, checker, and all backend emitters to support explicit exports and namespace-prefixed usage.
* **Done (2026-08-05):** Implemented an AST-level namespace resolver in ast.ResolveModules that scopes explicit (export) blocks and renames local definitions cleanly, while translating (use "file" as alias) imports into prefix-mangled symbols to remain compatible with existing backend behavior. Deprecated (include) and migrated tests/examples to (use). Renamed (import) to (go_import).
