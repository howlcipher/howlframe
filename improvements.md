...
| 66 | [Standalone Zero Runtime Phase 1: Architecture Decision](#66-standalone-zero-runtime-phase-1-architecture-decision) | Pending | 2.0 (8×1÷4) | — | — | gpt-5.6-sol | Rescoped 2026-07-31 from the single effort-8 "Standalone Zero Runtime Environment" item: decide the runtime architecture and prove it on one real construct end-to-end before committing to full-language coverage. |
...
### 72. Standalone Zero Runtime Phase 2: Full Language Cutover
* **Description:** Extend the architecture Phase 1 (`#66`) chose and proved on a minimal slice to cover the full language surface — strings, collections (`list`/`dict`, `map_get`/`list_get`), aggregates/structs, and the LLM-backed primitives' HTTP calls (`achieve`, `confidence`, `neural_circuit`, `llm_generate`, `fuzzy_cast`, `semantic_match`, `lazy_synthesize`) — and make the result a real, documented alternate (or eventual default) build target alongside the existing Go backend, not just a proof of concept.
* **Why:** Phase 1 is deliberately scoped to "decide and prove," not "ship." Splitting the work this way (mirroring how `#53`/`#62` phased the IR abstraction) keeps Phase 1 small and honestly scored instead of quietly absorbing the whole effort-8 item, while keeping this phase explicitly blocked on — not parallel to — Phase 1's architecture decision, so the two items can't drift onto different runtime choices.
* **Impact:** 8/10 (High — this is the phase that actually delivers "true standalone binaries," the original item's stated goal).
* **Blocked on:** `#66` (Phase 1) landing, specifically its architecture-decision Done note. Do not start this phase's implementation before that note exists.
* **Fix sketch:** Once Phase 1 lands, audit every primitive currently emitted by the Go backend (`internal/backend/gogen/gogen.go`) against what the chosen standalone architecture supports, and close gaps in priority order: collections and control flow first (needed by nearly every fixture), then the LLM-backed primitives' HTTP call plumbing (needs an HTTP client available without Go's runtime — a real architecture-level question for Phase 1 to have already answered), then string/regex/exec/db primitives last as the long tail. Re-run the full `tests/*.zero` fixture sweep against the standalone target at each milestone, the same way the Go backend fixture sweep is used elsewhere in this backlog.
